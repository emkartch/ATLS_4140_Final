...
func shuffle_tiles():
	...
	for t in range(0,100):
		...
		if tiles[tile] != $Tile16 and tile != previous and tile != previous_1:
			var rows = int(tiles[tile].position.y / 246)
			var cols = int(tiles[tile].position.x / 246)
...
func _process(delta):
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and mouse:
		...
		mouse = false
		var rows = int(mouse_copy.position.y / 246)
		var cols = int(mouse_copy.position.x / 246)
		check_neighbours(rows,cols)
		if tiles == solved:
...
func find_empty(position,pos):
	var new_rows = int(position.y / 246)
	var new_cols = int(position.x / 246)
